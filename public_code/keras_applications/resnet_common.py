...
def block2_basic(x, filters, kernel_size=3, stride=1,
    ...
    x = layers.Conv2D(filters, kernel_size, strides=stride, use_bias=False,
                      name=name + '_1_conv')(x)
    x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5,
                                  name=name + '_1_bn')(x)
...
def stack2_basic(x, filters, blocks, stride1=2, activation='relu', name=None):
    """A set of stacked residual blocks.
    ...
    # Returns
        Output tensor for the stacked blocks.
    """
    x = block2_basic(x, filters, conv_shortcut=True, activation=activation, name=name + '_block1')
    for i in range(2, blocks):
        x = block2_basic(x, filters, activation=activation, name=name + '_block' + str(i))
    x = block2_basic(x, filters, stride=stride1, activation=activation, name=name + '_block' + str(blocks))
    return x
...
def ResNet34(include_top=True,
             ...
             **kwargs):
    def stack_fn(x):
        x = stack1_basic(x, 64, 3, stride1=1, name='conv2')
        x = stack1_basic(x, 128, 4, name='conv3')
        x = stack1_basic(x, 256, 6, name='conv4')
        x = stack1_basic(x, 512, 3, name='conv5')
        return x
...
def ResNet34V2(include_top=True,
               ...
               **kwargs):
    def stack_fn(x):
        x = stack2_basic(x, 64, 3, name='conv2')
        x = stack2_basic(x, 128, 4, name='conv3')
        x = stack2_basic(x, 256, 6, name='conv4')
        x = stack2_basic(x, 512, 3, stride1=1, name='conv5')
        return x
...
def ResStack2(x, filters, blocks, stride1=2, activation='relu', name=None, **kwargs):
    global backend, layers, models, keras_utils
    backend, layers, models, keras_utils = get_submodules_from_kwargs(kwargs)
    return stack2(x, filters, blocks, stride1=stride1, activation=activation, name=name)